...
title: |
  ...
  | &nbsp;
  | &nbsp;
  | USFWS, North Atlantic-Appalachian Region SET Data Analysis
  | &nbsp;
  | &nbsp;
  | &nbsp;
...
opts_knit$set(root.dir = '/Users/zach/Dropbox (ZachTeam)/Projects/SET_USFWS_Region_5/Reports/RMarkdown_pdf')
...
source("./R_source/formatSETdataR5.R")
source("./R_source/formatMHdataR5.R")
...
2)	We analyzed data collected at SET stations (*N* = 58) located within monitoring sites (*N* = 102; Fig. 1) among  NWRs  within the USFWS North Atlantic-Appalachian (Table 1). 
...
#my working dir
mydir<-"/Users/zach/Dropbox (ZachTeam)/Projects/SET_USFWS_Region_5/Reports/RMarkdown_pdf"
...
set.data.formatted<-try(formatSETdataR5(dataIn=raw.set.data, dataDir=mydir, IncludeUncorrected = TRUE))
...
mh.data.formatted<-try(formatMHdataR5(dataIn=raw.mh.data, dataDir=mydir))
...
#### Table 1. List of 102 sites among 16 US Fish and Wildlife Service National Wildlife Refuge (NWR) units included in SET data analyses along with corresponding State, count of SET stations, count of unique observers, duration of years, and starting and ending years of data collection. 
```{r, message=FALSE, warning=FALSE}
...
#### Figure 1. Study Area showing sites (*N* = 102) where SET benchmarks are located among 16 USFWS National Wildlife Refuges, North Atlantic-Appalachian Region. 
```{r, warning=FALSE, message=FALSE, fig.width=12, fig.height=12, dpi=300}
#Map with study area locations

length(unique(set.data$Refuge))

#get locations of SETs
SETcoords<-unique(set.data[, c("State","Refuge","Site_Name","Station_Name","Longitude","Latitude")])

#number of SET stations
# length(unique(set.data$Site_Name))
# length(unique(set.data$Refuge))

#aggregate to get Site coords
site.long<-aggregate(Longitude~Site_Name+Refuge+State, data=SETcoords, FUN=mean)
site.lat<-aggregate(Latitude~Site_Name+Refuge+State, data=SETcoords, FUN=mean)
site.coords.merge<-merge(site.long, site.lat, by=c("Site_Name","Refuge","State"),all=TRUE)

#Make State a factor
site.coords.merge$State<-as.factor(site.coords.merge$State)
...
studyAreaMap<-ggmap(basemap, darken = c(0.6, "white"))+
  geom_point(data=site.coords.merge, aes(x=Longitude, y=Latitude,color=State),size=2.5,alpha=0.8)+
  ...
  geom_text_repel(data=site.coords.merge,aes(x=Longitude,y=Latitude, label=Site_Name, color=State), show.legend=TRUE, size=3, segment.alpha=0.7, bg.color = "white", bg.r = 0.16)+
  guides(fill = guide_legend(title = "National Wildlife Refuge", label = TRUE))+
  ggsn::scalebar(x.min=mapExtent$ll.lon, x.max=mapExtent$ur.lon, y.min=mapExtent$ll.lat, y.max=mapExtent$ur.lat,transform=TRUE, dist_unit="km",dist=100, st.dist=0.015, height = 0.01,anchor=c(x=-69,y=36.7))
...
```{r, fig.width=7, fig.height=7, dpi=150}
...
#### Figure 3. Plot change in elevation data relative to initial height (mm) over time with SETs with fewer than 5 years of data omitted. Orange rergression line is a linear model from 2009 to 2021.
```{r, message=FALSE,fig.width=7, fig.height=7, dpi=150}
...
set.data.SETomit<-subset(set.data.all, ! Station_Name %in% setOmit)
  ...
  SETallDatesPlot_SETomit<-ggplot(data=set.data.SETomit, aes(x=Date, y=value, group=1))+
    ...
    #geom_path(aes(color=Station_Name),alpha=0.2)+
    geom_smooth(method=lm, fullrange=TRUE, linetype=1, color="gray20", se=TRUE)+
    geom_smooth(data=subset(set.data.SETomit, Year > 2008),method=lm, fullrange=FALSE, linetype=1, color="orange", se=TRUE)+
    scale_y_continuous(expand=c(0.1,0.1))+
    scale_x_date(date_breaks = "5 year",date_labels = "%Y")+
        theme(panel.background = element_rect(color="black",fill="transparent"),
          panel.border = element_rect(color="black", fill="transparent"),
...
#now, break set.data.merge into two pieces

#get list of site Names
listA<-siteList[1:(length(siteList)/2)]
listB<-siteList[((length(siteList)/2)+1):length(siteList)]

#subset the data using listA and listB
set.data.merge.A<-subset(set.data.merge, Site_Name %in% listA)
set.data.merge.B<-subset(set.data.merge, Site_Name %in% listB)

mySigColors<-c("royalblue1","gray20","tomato")
mySigFills<-c(alpha("royalblue1",0.3), alpha("gray20",0.3),alpha("tomato",0.3))

  SETallDatesPlot_SETomit<-ggplot(data=set.data.merge.A, aes(x=Date, y=value, group=1))+
...
#### Figure 4 (cont'd). Station-level change in elevation data relative to initial height (mm) over time.
```{r, message=FALSE, fig.width=16, fig.height=20}

  SETallDatesPlot_SETomit<-ggplot(data=set.data.merge.B, aes(x=Date, y=value, group=1))+
    geom_point(color="gray50",alpha=0.7, size=0.8, shape=16)+
    #geom_path(aes(color=Station_Name),alpha=0.2)+
    geom_smooth(aes(color=Sig_Trend, fill=Sig_Trend),method=lm, fullrange=TRUE, linetype=1, se=TRUE)+
    scale_color_manual(values=mySigColors)+
...
#### Figure 6. SET Station-level trend in change in wetland elevation (mm/year). Box-and-whiskar plots show median values with inter-quartile ranges and whiskars show 99.3% CIs
...
trendMapRegion<-ggmap(basemap,darken = c(0.7, "gray20"))+
  geom_point(data=site.order.merge, aes(x=Longitude, y=Latitude, color=mean), size=2.5,alpha=0.8, position=position_jitter(width=0.1, height=0.1))+
  ...
  # geom_label_repel(data=site.coords.merge,aes(x=Longitude,y=Latitude, label=Site_Name), fill="gray30",color="white", show.legend=FALSE, label.padding=0.4, size=6, segment.alpha=0.7, alpha=0.8)+
    geom_text_repel(data=site.order.merge,aes(x=Longitude,y=Latitude, label=Site_Name, color=mean), show.legend=FALSE, size=4, segment.alpha=0.7,
...
for(i in 1:length(siteList)){
  ...
  ###############################################################################
  #hard code some settings by refuge if needed
  #set zoom  (need to fix this for R5 sites)
  # plotZoom=ifelse(siteName=="Harris Neck", 11, 
  #                 ifelse(siteName=="Cape Romain - Horsehead Key",14,17))
  plotZoom = ifelse(siteName=="Unit II",14,14)
  ...
  plotScaleAnchorX=0.99985
  plotScaleAnchorY=0.9997
...
trendMapSite<-ggmap(basemap,darken = c(0.2, "gray10"))+
  ...
  geom_label_repel(data=new.SETcoords,aes(x=Longitude,y=Latitude, label=Station_Name), fill="white",color="black", show.legend=FALSE,point.padding=0.8, label.padding=0.4, size=3, segment.alpha=0.7, alpha=0.8)+
...
if(!file.exists(myFile)){
  download.file("https://tidesandcurrents.noaa.gov/sltrends/data/USStationsLinearSeaLevelTrends.csv", destfile = paste("/Users/zach/Dropbox (ZachTeam)/Projects/SET_USFWS_Region_5/Reports/RMarkdown_pdf/Data/SLR_trend_data/",paste("USStationsLinearSeaLevelTrends", Sys.Date(),".csv", sep=""),sep=""))
}
...
### Figure 8. NOAA sea-level trend estimates (mm/yr) among SET sites. NOAA station ID numbers are shown in black.
```{r, fig.width=10, fig.height=13, dpi=300}
...
slr.trend.sort<-unique(slr.trend.sort)


#remove those Unit II from all data
slr.trend.sort.sub<-subset(slr.trend.sort, Site_Name != "Unit II")

#check Unit B
slr.unitII<-subset(slr.trend.sort, Site_Name=="Unit II")

#now, take the mean
slr.unitII.mean<-mean(slr.unitII$Mean_SLR_Trend_mm_per_yr)

#build data.frame
slr.unitII.df<-slr.unitII[1,]
slr.unitII.df$Mean_SLR_Trend_mm_per_yr<-slr.unitII.mean

#combine
slr.trend.out<-rbind(slr.trend.sort.sub, slr.unitII.df)

#make Station.ID a factor
slr.trend.out$Station.ID<-as.factor(as.character(slr.trend.out$Station.ID))

#plot annual mean sea.level and trend (blue and red plot)
site_SLR_plot_1<-ggplot(data=slr.trend.out, aes(x=Site_Name, y=Mean_SLR_Trend_mm_per_yr))+
  #geom_hline(yintercept=0, color="gray80")+
  geom_bar(stat="identity",aes(fill=as.factor(Station.ID)),alpha=0.4, name="Station.ID")+
   ...
   coord_flip()+
   geom_text(aes(label = myLabel), hjust = 1.4, color="black",size=3) +
...
names(SET.gnss.elev)[names(SET.gnss.elev)=="StationCode"]<-"Station_Code"
...
#fix names
names(slr.trend.data.merge.2)[names(slr.trend.data.merge.2)=="Latitude.x"]<-"Latitude"
names(slr.trend.data.merge.2)[names(slr.trend.data.merge.2)=="Longitude.x"]<-"Longitude"
...
#NEED VERTICAL OFFSET values for each State in R5 data

#vertical offset (Will need this info from Region 5, potentially?)
vertical.offset.state<-data.frame("State"=c("MA", "DE", "VA", "NJ", "ME", "NY", "CT"),
                                  "Vertical_Offset_mm"=c(394,394,394,394,394,394,394))
...
#fix columns
names(raw.set.data)[names(raw.set.data)=="StationCode"]<-"Station_Code"


#pin data
pin.data<-unique(raw.set.data[,c("Refuge","Site_Name","Station_Code","EventDate","PipeDirectionAzimuth","PinPosition","PinLength_mm","PinHeight_mm_Uncorrected","PinHeight_mm","DataProcessingLevelCode")])

#Since all PinHeight_mm are NA, copy PinHeight_mm_Uncorrected to PinHeight_mm column
pin.data$PinHeight_mm<-pin.data$PinHeight_mm_Uncorrected
...
pin.data.accepted.2<-merge(pin.data.accepted.1, site.elev.keep.df.2,
                           by=c("Refuge","Site_Name","Station_Code"),
                           all.x=TRUE)
...
################################################################33
#set PinLength to 761 for all pins?
pin.data.accepted.2$PinLength_mm<-761
################################################################33
...
pin.height.computed<-subset(pin.data.accepted.2, ! is.na(True_Pin_Height_mm))
...
#remove NAs where there's no SLR data
pin.height.computed.sub<-subset(pin.height.computed, ! is.na(Mean_SLR_Trend_mm_per_yr))


#aggregate to get mean pin height per yr
yearly.elevation.mean<-aggregate(True_Pin_Height_m~Year+Refuge+Station_Name+Station_Code, data=pin.height.computed.sub, FUN="mean")
colnames(yearly.elevation.mean)<-c("Year","Refuge","Station_Name","Station_Code","Mean_True_Pin_Height_m")

yearly.elevation.sd<-aggregate(True_Pin_Height_m~Year+Refuge+Station_Name+Station_Code, data=pin.height.computed.sub, FUN="sd")
colnames(yearly.elevation.sd)<-c("Year","Refuge","Station_Name","Station_Code","SD_True_Pin_Height_m")
...
combined.df.2$Year<-as.integer(round(combined.df.2$Year,0))

#plot annual mean sea.level and trend (blue and red plot)
sea.level.plot<-ggplot(data=combined.df.2, aes(x=as.integer(Year), y=Mean))+
  geom_hline(yintercept=0, color="gray80")+
...
geom_line(data=subset(combined.df.2,Type=="Surface_Elevation_m"),aes(x=Year, y=Mean,group=Station_Name),color="goldenrod2",alpha=0.2,inherit.aes = FALSE)+
  ...
  scale_x_continuous(breaks=c(2010,2012,2014,2016,2018,2020),
                     labels =c("2010","2012","2014","2016","2018","2020"),limits=c(2010,2021))+
    #xlim(2010,2020)+
...
sea.level.plot<-ggplot(data=combined.df.4, aes(x=as.integer(Year), y=Mean))+
  geom_hline(yintercept=0, color="gray80")+
  ...
  scale_x_continuous(breaks=c(2010,2015,2020),
                     labels =c("2010","2015","2020"),limits=c(2010,2021))+
...
mh.site.trend.plot<-ggplot(data=mh.site.order, aes(x=Site_Name, y=mean))+
  #coord_flip()+
  ...
  #scale_fill_gradient2(low="slateblue",mid="gray60",high="seagreen4")+
    scale_fill_gradient2(low="tomato",mid="gray60",high="royalblue1" , midpoint=0)+
  scale_color_gradient2(low="tomato",mid="gray60",high="royalblue1" , midpoint=0)+
theme(panel.background = element_rect(fill="white"),
        panel.border=element_rect(fill="transparent",color="black"),
        axis.text.x=element_text(angle=90,  vjust=0.5, hjust=1),
...
mh.station.trend.plot<-ggplot(data=mh.station.order, aes(x=Station_Name, y=mean))+
  #geom_bar(stat="identity",aes(fill=mean),alpha=0.9)+
  geom_point(aes(color=mean),alpha=0.9,size=2)+
  geom_errorbar(aes(ymin=mean-SE, ymax=mean+SE, color=mean),width=0, show.legend=FALSE)+
  geom_hline(yintercept = 0, linetype=2, color=alpha("lightgray",0.8))+
...
mh.data.comb<-mh.data.formatted
...
mh.data.comb.sub.2$Station_Date_Year<-paste(mh.data.comb.sub.2$Station_Name, mh.data.comb.sub.2$Date, mh.data.comb.sub.2$Year, sep="+")
...
mh.covs<-read.table(text=as.character(mh.station.summary$Station_Date_Year),sep="+")
...
#create Station_Year column 
mh.station.summary.2$Station_Year<-paste(mh.station.summary.2$Station_Name, mh.station.summary.2$Year, sep="+")
...
#remove problematic data from 
#set.data.comb<-try(subset(set.data.comb, ! Site_Name  %in% removeSiteList))


#create Station_Date_Year column
set.data.comb$Station_Date_Year<-paste(set.data.comb$Station_Name, set.data.comb$Date, set.data.comb$Year, sep="+")
...
#get covs
set.covs<-read.table(text=as.character(set.station.summary.2$Station_Date_Year), sep="+")
...
set.station.summary.3$Station_Year<-paste(set.station.summary.3$Station_Name, set.station.summary.3$Year, sep="+")
...
mh.set.comb$Station_Type<-paste(mh.set.comb$Station_Name, mh.set.comb$Data_Type, sep="+")
...
```{r, fig.width=10, fig.height=12, dpi=150,results='asis'}

#merge with data to add Site_Name
site.station.df<-unique(set.data[,c("Refuge","Site_Name","Station_Name")])

mh.set.comb<-merge(mh.set.comb, site.station.df, by="Station_Name",all.x=TRUE)

#divide mh.set.comb in 2 pieces
mhStationList<-unique(mh.set.comb$Station_Name)

#Make 3 lists, get list of site Names
mhlistA<-mhStationList[1:floor(length(mhStationList)/5)]
mhlistB<-mhStationList[(floor(length(mhStationList)/5)+1):(floor(length(mhStationList)/5)*2)]
mhlistC<-mhStationList[(floor(length(mhStationList)/5)*2+1): (floor(length(mhStationList)/5)*3)]
mhlistD<-mhStationList[(floor(length(mhStationList)/5)*3+1): (floor(length(mhStationList)/5)*4)]
mhlistE<-mhStationList[(floor(length(mhStationList)/5)*4+1): (floor(length(mhStationList)/5)*5)]

#subset the data using listA and listB
mh.set.comb.A<-subset(mh.set.comb, Station_Name %in% mhlistA)
mh.set.comb.B<-subset(mh.set.comb, Station_Name %in% mhlistB)
mh.set.comb.C<-subset(mh.set.comb, Station_Name %in% mhlistC)
mh.set.comb.D<-subset(mh.set.comb, Station_Name %in% mhlistD)
mh.set.comb.E<-subset(mh.set.comb, Station_Name %in% mhlistE)

#figure here showing computed SET-MH values indicating sub-surface influence on accretion/subsidence patterns.

  SET_MH_allDatesPlot.2<-ggplot(data=mh.set.comb.A, aes(x=Date, y=mean, group=Station_Type))+
    geom_hline(yintercept=0, linetype=2, color="gray30",alpha=0.6)+
    geom_errorbar(aes(ymin=mean-SE, ymax=mean+SE,color=Data_Type),alpha=0.5)+
    geom_point(aes(color=Data_Type), size=1, shape=16,alpha=0.5)+
    geom_path(aes(color=Data_Type),alpha=0.5)+
...
#### Figure 14 (cont'd). Plot of relative changes (mean $\pm$ SE) in elevation (mm) for station-level accretion (red) and SET (light blue) data for each station.
```{r, fig.width=10, fig.height=13, dpi=150,results='asis'}

  SET_MH_allDatesPlot.2<-ggplot(data=mh.set.comb.B, aes(x=Date, y=mean, group=Station_Type))+
    geom_hline(yintercept=0, linetype=2, color="gray30",alpha=0.6)+
    geom_errorbar(aes(ymin=mean-SE, ymax=mean+SE,color=Data_Type),alpha=0.5)+
    geom_point(aes(color=Data_Type), size=1, shape=16,alpha=0.5)+
    geom_path(aes(color=Data_Type),alpha=0.5)+
...
#### Figure 14 (cont'd). Plot of relative changes (mean $\pm$ SE) in elevation (mm) for station-level accretion (red) and SET (light blue) data for each station.
```{r, fig.width=10, fig.height=13, dpi=150,results='asis'}
  SET_MH_allDatesPlot.2<-ggplot(data=mh.set.comb.C, aes(x=Date, y=mean, group=Station_Type))+
    geom_hline(yintercept=0, linetype=2, color="gray30",alpha=0.6)+
    geom_errorbar(aes(ymin=mean-SE, ymax=mean+SE,color=Data_Type),alpha=0.5)+
    geom_point(aes(color=Data_Type), size=1, shape=16,alpha=0.5)+
    geom_path(aes(color=Data_Type),alpha=0.5)+
...
#### Figure 14 (cont'd). Plot of relative changes (mean $\pm$ SE) in elevation (mm) for station-level accretion (red) and SET (light blue) data for each station.
```{r, fig.width=10, fig.height=13, dpi=150,results='asis'}
  SET_MH_allDatesPlot.2<-ggplot(data=mh.set.comb.D, aes(x=Date, y=mean, group=Station_Type))+
    geom_hline(yintercept=0, linetype=2, color="gray30",alpha=0.6)+
    geom_errorbar(aes(ymin=mean-SE, ymax=mean+SE,color=Data_Type),alpha=0.5)+
    geom_point(aes(color=Data_Type), size=1, shape=16,alpha=0.5)+
    geom_path(aes(color=Data_Type),alpha=0.5)+
...
SET_MH_allDatesPlot.2<-ggplot(data=mh.set.comb.E, aes(x=Date, y=mean, group=Station_Type))+
    geom_hline(yintercept=0, linetype=2, color="gray30",alpha=0.6)+
    geom_errorbar(aes(ymin=mean-SE, ymax=mean+SE,color=Data_Type),alpha=0.5)+
    geom_point(aes(color=Data_Type), size=1, shape=16,alpha=0.5)+
    geom_path(aes(color=Data_Type),alpha=0.5)+
...
```{r, echo=FALSE, message=FALSE, results = "hide"}
  ...
  set.data.SETomit$Station_Year<-paste(as.character(set.data.SETomit$Station_Name), set.data.SETomit$Year,sep="+")
  ...
  computeTrendsSET(set.data.SETomit)
...
formatSETdataR5<-function(dataIn, dataDir,
  ...
  SETcoords.1<-read.csv(file=paste(myDir,
                                   "/Data/SET Stations.csv",sep="")
                        , header=TRUE, skip=3)
  
  #get data.frame of StationCode and StationName
  stationName.df<-unique(SETcoords.1[,c("StationName",
                                        "StationCode")])
  
  #merge with new.data
  new.data<-merge(new.data, stationName.df, by="StationCode",
                  all.x=TRUE)
  ...
  new.data$PipeDirectionCode<-gsub("1","A",gsub("3","B",
                                                gsub("5","C",gsub("7","D",new.data$PipeDirectionCode))))
  ...
  ifelse(isTRUE(IncludeUncorrected),
                   ...
                   new.data<-subset(new.data, 
                                    c(! is.na(PinHeight_mm) & 
                              ! is.na(PinHeight_mm_Uncorrected)))
                   )
  
  #change DataProcessingLevelCode to DataProcessingLevelLabel
  names(new.data)[names(new.data)=="DataProcessingLevelCode"]<-
    "DataProcessingLevelLabel"
  ...
  ################################
  #are there double observers?  Yes, however, 
  #it appears the same observer made two measurements 
  #during a single sampling event, so these can't help 
  #us estimate observer-based measurement bias).
  
  doubObs<-aggregate(ReaderFullName~Station_Name+
                       EventDate, data=new.data, FUN=length)
  #check if there are cases with > 36 observations 
  #(i.e., 4 arms X 9 pins = 36), if so indicates two observers, 
  #typically.
  doubObsSub<-subset(doubObs, ReaderFullName >36)
  
  ################################
  #Issue found on 11-24-2020 where replicate rows have different 
  #DataLevelProcessingCode factor levels, so they are being 
  #included within the data as independent records.  
  #Fixing this now in the formatSETdataR4 funciton.
  
  #get list of replicits
  replicateList<-paste(doubObsSub$Station_Name, 
                       doubObsSub$EventDate,sep="_")

  #get these rows that have apparent replicated rows with 
  #different DataLevelProcessingCodes
  new.data$Station_Event<-paste(new.data$Station_Name, 
                                new.data$EventDate,sep="_")
  replicate.data<-subset(new.data, Station_Event %in% 
                           replicateList)

  replicate.data.sub<-subset(replicate.data, 
                             ObservationTypeCode != "R")
  ...
  #################################################################################################
  #create and save table of data omitted from Analysis: 
  #Provisional and Raw data types
  ...
  ifelse(isTRUE(IncludeUncorrected),
         ...
         colnames(data.omitted)<-c("Refuge","Site_Name",
                                   "Station_Name","EventDate","ReaderFullName","DataProcessingLevelCode")
         },
         data.omitted<-subset(new.data.1, 
                              c(DataProcessingLevelCode != "A")))
  
  
  #simplify
  data.omitted.out<-unique(data.omitted[,c("Refuge","Site_Name",
                                           "Station_Name","EventDate","ReaderFullName","DataProcessingLevelCode")])
  ...
  all.stations.pipe.dirs<-unique(new.data.1[,c("Refuge",
                                               "Site_Name",
                                               "Station_Name", "PipeDirectionAzimuth","PipeDirectionCode")])
  
  #add factor for pipe direction for each SET
  all.stations.pipe.dirs$Station_Name<-
    as.factor(all.stations.pipe.dirs$Station_Name)
  ...
  #melt and cast data to create column with list of 
  #PipeAzimuth directions
  ...
  all.stations.pipe.cast<-as.data.frame(data.table::dcast(Refuge+
                              Site_Name+Station_Name~PipeDirectionCode, 
                          data=all.stations.pipe.melt, value.var="value"))
  ...
  all.stations.pipe.cast$PipeDirectionAzimuths<-
    paste(all.stations.pipe.cast$'A', all.stations.pipe.cast$'B', 
          all.stations.pipe.cast$'C', all.stations.pipe.cast$'D',
          sep=", ")
  ...
  #save csv file
  write.csv(all.stations.pipe.out, file=paste(paste(myDir,
                                                    "Data",
                                                    "Tables",
                                                    sep="/"),
  ...
  missing.pin.df<-subset(new.data, c(is.na(PinHeight_mm) | 
                                    is.na(PinHeight_mm_Uncorrected)))
  
  #simplify
  missing.pin.df.sub<-unique(missing.pin.df[,c("Refuge","Site_Name",
                                               "Station_Name","EventDate","PipeDirectionAzimuth","PinPosition",
                                               "PinLength_mm",
                                               "PinLength_mm_Baseline",
                                               "PinHeight_mm_Uncorrected", 
                                               "PinHeight_mm")])
  ...
  data.2$station.position.year<-paste(data.2$Station_Name, 
                                      data.2$Position_Name, 
                                      data.2$Year,sep=".")
  data.2$station.year<-as.factor(paste(data.2$Station_Name, 
                                       data.2$Year,sep="."))
  ...
  #Create column for visits by each factor level of 
  #SET stations (Station_Name).
  #Use 'data.table' package to add sequential visit numbers by 
  #each factor (i.e., Station_Name)
  #Truncate data.frame
  data.2.events<-data.2[,c("Station_Name","Year","Month","Day",
                           "ord.Day","station.year","station.year.ord.day")]
  ...
  data.2.events<-data.2.events[order(data.2.events$Station_Name, 
                                     data.2.events$Year, data.2.events$ord.Day)]
  ...
  data.2.events<-data.2.events[,c("Station_Name","Year","Month",
                                  "Day","ord.Day","Visit")]
  ...
  data.out$VisitPad<-ifelse(nchar(as.character(data.out$Visit))<2, 
                              paste("0",as.character(data.out$Visit),
                                    sep=""),
  ...
  data.out.sub<-unique(data.out[,c("Refuge","Site_Name","Station_Name",
                                   "Year","Month","Day","EventDate","Visit","year.visit","station.year",
  ...
  #melt
  num.visits.melt<-data.table::melt(num.visits.agg.dt,
                                    id.vars=c("Refuge","Site_Name",
                                              "Station_Name","Year"),
                                    measure.vars=c("EventDate")
  )
  
  #cast
  num.visits.cast<-as.data.frame(data.table::dcast(Refuge+Site_Name+
                                                     Station_Name~Year, 
  ...
  write.csv(num.visits.cast, file=paste(paste(myDir,"Data","Tables",sep="/"),
                                        "Table_all_SET_num_visits_per_year.csv",sep="/"),row.names=FALSE)
  ...
  ################################################################################
  #Use package 'data.table' (reshape is depreciated) to reorganize 
  #data (like Pivot table in Excel) using melt().
  ...
  unique(data.out.keep.dt$Position_Name) #Problem here! more than 4 
  #factors for Pipe Directions!
  
  #find data.out.keep.dt NAs position name
  postionName.NA<-subset(data.out.keep.dt, is.na(Position_Name))
  
    data.melt<-data.table::melt(data.out.keep.dt, id=c("Refuge","Station_Name","ReaderFullName","Year","Position_Name",
                                                       "PipeDirectionAzimuth","PinPosition","Visit","year.visit"),
                                measure=c("PinHeight_mm"))
  ###################################################################
    #Generate lists 
      
    #Get stationList. (FYI, using the "<<-" sign puts object into 
      ...
      for(j in 1:length(positionList)){
        ...
        setDT(position.data.dt)
          ...
          cast.data<-as.data.frame(data.table::dcast(position.data.dt, 
                                                     PinPosition~year.visit+
                                                       variable, 
                                                     value.var="value"))
          
          #replace NAs with imputed means (among pins)
          for(i in 1:ncol(cast.data)){
            cast.data[is.na(cast.data[,i]), i] <- mean(cast.data[,i], 
                                                       na.rm = TRUE)
          }
...
data.out.keep.1<-unique(data.out.keep[,c("Refuge","Site_Name",
                                         "Station_Name","EventDate",
                                         "Year","Month","Day","ord.Day", "Visit","year.visit","ReaderFullName","ReaderID","PipeDirectionAzimuth",
                                         "Position_Name",
...
R5_coords.sub<-unique(R5_coords[,c("State","StationName","StationCode",
                                   "Latitude","Longitude")])
colnames(R5_coords.sub)<-c("State","Station_Name","Station_Code",
                           "Latitude","Longitude")
...
new.data.out.2<-new.data.out.1
  ...
  write.csv(data.out.3, file=paste(myDir,"Data",
                            paste(paste("All_R5_SET_data_formatted_",
                                               Sys.Date(),sep=""),
                                         "csv",sep="."),sep="/"),
            row.names=FALSE)
  ...
  #create Date column
  set.data$Date<-as.Date(set.data$EventDate, 
                         tryFormats = c("%m-%d-%Y", "%m/%d/%Y"))
  
  set.delta.data<-set.data[,c("State","Refuge","Site_Name",
                              "Station_Name","Station_Code",
                              "Date","EventDate","Year","Month",
                              "Day","ord.Day", "Visit","year.visit",
                              "ReaderFullName","ReaderID",
                              "PipeDirectionAzimuth","Position_Name",
                              "Plot_Year_Visit_Position",
                              "Longitude","Latitude","deltaPin1",
                              "deltaPin2","deltaPin3",
                              "deltaPin4","deltaPin5","deltaPin6", "deltaPin7","deltaPin8","deltaPin9")]

  #Rename columns.
  colnames(set.delta.data)<-c("State","Refuge","Site_Name",
                              "Station_Name","Station_Code",
                              "Date","EventDate","Year","Month",
                              "Day","ord.Day", "Visit","year.visit","ReaderFullName","ReaderID","PipeDirectionAzimuth",
  ...
  delta.set.melt.dt<-data.table::melt(set.delta.data.dt, id.vars=c("State","Refuge","Site_Name","Station_Name","Station_Code",
                                                                   "Date","EventDate","Year","Month","Day","ord.Day", "Visit","year.visit","ReaderFullName","ReaderID","PipeDirectionAzimuth",
                                                                   "Position_Name","Plot_Year_Visit_Position","Longitude","Latitude"),
...
#### Simple function to get summary statistics.
\  
...
formatMHdataR5<-function(dataIn, dataDir){
  ...
  #merge with new.data
  new.data<-merge(new.data, stationName.df, by="Station_Name",
                  all.x=TRUE)
  ...
  data.1$station.event.mh<-paste(data.1$Station_Name, 
                                 data.1$EventDateTimeWithOffset, 
                                 data.1$MarkerHorizonID,sep="_")
  
  #convert DateEstablished from yyyy-mm-dd to mm/dd/yyyy format
  data.1$DateEstablished<-paste(read.table(text=as.character(data.1$DateEstablished),
                                           sep="-")$V2,
                                read.table(text=as.character(data.1$DateEstablished),
                                           sep="-")$V3,
                                read.table(text=as.character(data.1$DateEstablished),
                                           sep="-")$V1,
                                sep="/")
  ...
  for(i in 1:length(markerHorizonList)){
    ...
    new.mh.data<-subset(data.1, 
                        MarkerHorizonID==markerHorizonList[i])
    ...
    for(j in 1:length(coreList)){
      ...
      new.label.data<-unique(subset(new.mh.data, 
                                    CoreObservationNumber == coreList[j]))
...
date.est<-data.frame("Refuge"=unique(new.label.data$Refuge),
  ...
  "EventDateTimeWithOffset" = unique(new.label.data$DateEstablished),
    ...
    "MarkerHorizonLabel" = unique(new.label.data$MarkerHorizonLabel), 
      ...
      "station.event.mh" = paste(unique(new.label.data$Station_Name), unique(new.label.data$DateEstablished),
                      unique(new.label.data$MarkerHorizonID),sep="_"))
      
      #add to new.label.data
      new.label.data.2<-rbind(date.est, new.label.data)
      
      #add Delta column (Difference of measurement 
      #from baseline (0)), to new.label.data.2
      new.label.data.2$Delta_MH_mm<-
        new.label.data.2$DepthToBenchmark_mm-0
  ...
  data.2$station.mh.year<-paste(data.2$Station_Name, 
                                data.2$MarkerHorizonID, 
                                data.2$Year,sep=".")
  data.2$station.year<-as.factor(paste(data.2$Station_Name, 
                                       data.2$Year,sep="."))
  data.2$station.year.ord.day<-as.factor(paste(data.2$Station_Name, 
  ...
  #check that each station has more than 1 visit
  message("Checking data for inconsistencies.")
  check.data<-aggregate(Year~MarkerHorizonID, data=data.out, 
                        FUN=function(x){length(unique(x))})
  check.data$enoughVisits<-ifelse(check.data[,2]>1,"Yes","No")
  ...
  #data.out$Station_Name<-as.character(data.out$Station_Name)
  data.out.keep<-data.out[data.out$MarkerHorizonID %in% 
                            keepMarkerHorizons,]
  data.out.keep$Station_Name<-
    as.factor(as.character(data.out.keep$Station_Name))

  names(data.out.keep)
  
  #fix NWR names
  #remove "National Wildlife Refuge" from RefugeName or 
  #Wildlife Managment Area
  data.out.keep$Refuge<-trimws(gsub("Wildlife Management Area","",
                                    gsub("National Wildlife Refuge","",
...
SETstates<-read.csv(file=paste(myDir,"/Data/R5_SET.csv",sep=""),
                    header=TRUE)
...
#Merge data.out.keep and Region.lookup.
  ...
  #Merge new.data.out with XYdata.
  data.out.2<-merge(data.out.keep, R5_coords.sub, by=c("Station_Name",
                                                       
                                                       "Station_Code"),all.x=TRUE)
  
  #Save data (as a .csv file).
  message("Saving SET with delta height data to .csv file.")
  write.csv(data.out.2, file=paste(myDir,"Data",
                                   paste(paste("All_R4_MH_data_formatted_",
                          Sys.Date(),sep=""),"csv",sep="."),sep="/"), 
            row.names=FALSE)
...
#### Function to compute trends in MH data.
  ...
  for(j in 1:length(refugeList)){
    ...
    for(k in 1:length(siteList)){
      ...
      for(i in 1:length(stationList)){
        ...
        mhCovs<-unique(new.data[,c("State","Refuge","Site_Name",
                                   "Station_Name","MarkerHorizonID",
                                   "Year","Latitude","Longitude")])
        
        message("Getting slope and intercept from delta at 
                each pin vs. time regressions.")
        regResults.out<-list()
        for(k in 1:length(mhList)){
          ...
          sub.dataCovs<-unique(sub.data[,c("State","Refuge",
                                           "Site_Name","Station_Name")])
          #add Start_Year and End_Year
          ...
          sub.data.sampling.event<-aggregate(DepthToBenchmark_mm~
                                               EventDateTimeWithOffset, 
                                             FUN=mean,data=sub.data)
          #rename columns
          colnames(sub.data.sampling.event)<-c("Date","Mean_Delta_Depth_mm")
          
          #make Date a Date object
          sub.data.sampling.event$Date<-
            as.Date(as.character(sub.data.sampling.event$Date), 
                                                tryFormats = c("%m/%d/%Y"))
          
          #make Year column
          sub.data.sampling.event$Year<-year(sub.data.sampling.event$Date)
          
          #linear model with Visit nested within Year, and if there 
          #is only 1 visit, fit just Year
          mod.1<-tryCatch({
        ...
        #get means for each position
        station.mean<-summaryFunction(dataIn=regResults.out, 
                                      factor="Station_Name", 
                                      response="slope")
        station.mh.out.1<-merge(station.mean, stationCovs, 
                                by="Station_Name")
        station.mh.out<-station.mh.out.1[,c("State","Refuge",
                                            "Site_Name","Station_Name","Latitude","Longitude","Start_Year",
                                            "End_Year","n","mean",
                                            "var","SD","SE","CV",
                                            "lwr","upr")]
      ...
      site.merge.1<-merge(site.slopes, siteCovs, by="Site_Name")
      site.merge<-site.merge.1[,c("State","Refuge","Site_Name",
                                  "Start_Year","End_Year", "n","mean","var","SD","SE","CV","lwr","upr")]
