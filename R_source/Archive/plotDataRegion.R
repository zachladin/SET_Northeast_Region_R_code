#get regionList
regionList<-sort(unique(data.set.melt$RegionName))

#subset data
new.data.sub<-subset(data.set.melt, RegionName==regionList[1])


plotDataRegion<-function(dataIn){

  dataIn=new.data.sub
  new.data<-dataIn

  #gather info from data
  regionName<-sort(unique(as.character(new.data$RegionName)))

  #unitList
  unitList<-sort(unique(as.character(new.data$Unit_Code)))

  #find out how many sampling locations
  stationList<-sort(unique(as.character(new.data$Plot_Name)))
  nStations<-length(stationList)
  ...
  summary.out<-data.frame(RegionName=regionName,Unit_Code="All",Mean=global.mean, SD=global.sd, SE=global.se)

  #get means by units
  unit.out<-list()
  for(i in 1:length(unitList)){
    sub.data<-subset(new.data, Unit_Code==unitList[i])

    #get stationList

    new.summary<-summaryFunction(DataIn=sub.data, response="value",factor="year.visit")
    new.summary$Year<-as.integer(substr(new.summary$year.visit,1,4))
    new.summary$year.visit<-as.factor(new.summary$year.visit)
    new.summary$Unit_Code<-unitList[i]
    unit.out<-rbind(unit.out, new.summary)
  }

  unit.summary<-summaryFunction(DataIn=new.data, response="value",factor="year.visit")
  unit.summary$Year<-as.integer(substr(delta.summary$year.visit,1,4))
  unit.summary$year.visit<-as.factor(delta.summary$year.visit)
