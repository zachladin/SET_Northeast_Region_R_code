...
formatSETdataR4<-function(dataIn, dataDir,IncludeUncorrected=TRUE){
  ...
  ##############################
  #fix issue with variable pin lengths used at Pea Isand, Alligator River-Pocosin, and Roanoke River
  alternativePinLengths<-read.csv("./Data/PinLengths_update.csv")
  
  #get unique Station_Name from alternativePinLengths
  alt_stationList<-unique(alternativePinLengths$Station_Name)
  
  #subset to get sites where alternative pin lengths were used
  new.data.AltPins<-subset(new.data, Station_Name %in% alt_stationList)
  new.data.AltPins$PinPosition<-as.character(new.data.AltPins$PinPosition)
  
  #Look at any data where PinLength_mm is NA
  new.data.PinLengthNA<-subset(new.data, is.na(PinLength_mm))
  
  #save as .csv
  write.csv(new.data.PinLengthNA,"./Data/R4_SET_check_pinLengths.csv",row.names=FALSE)
  
  #save all data with PinLength_mm > 600
  new.data.HighPinLengths<-subset(new.data, PinHeight_mm_Uncorrected > 500)
  
  #save as .csv
  write.csv(new.data.HighPinLengths, "./Data/R4_SET_pinHeights_>500.csv",row.names=FALSE)
  
  #################################################################################################
  #Assume PinHeight_mm = 761 and replace all missing PinHeight_mm NAs with Uncorrected values
  
  ifelse(isTRUE(IncludeUncorrected),
         new.data$PinHeight_mm<-ifelse(is.na(new.data$PinHeight_mm),
                                               new.data$PinHeight_mm_Uncorrected,
                                               new.data$PinHeight_mm),
         new.data<-new.data)
